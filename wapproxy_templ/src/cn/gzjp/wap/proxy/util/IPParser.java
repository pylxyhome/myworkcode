...
public class IPParser {
	...
	private String DbPath = "/QQWry.dat"; // 纯真IP数据库地址
	...
	public void seek(String ip) throws Exception {
		this.IPN = ipToInt(ip);
		String lib = Configure.getConfig().getIpparserfile();
		log.info("lib file=" + lib);
		fis = new RandomAccessFile(lib, "r");
		buff = new byte[4];
		fis.seek(0);
		fis.read(buff);
	...
	public String getProv(String ip) {
		String res = "";
		String cnty = "";
		try {
			this.seek(ip);
			cnty = this.getCountry();
		} catch (Exception e) {
			log.info("getProvExcep:" + e.getMessage());
			e.printStackTrace();
			return res;
		}
		if(null == cnty){
		...
		int endIx = cnty.indexOf("省") + 1;
		if(endIx == 0){
			endIx = cnty.indexOf("市") + 1;
		}
		if(endIx == 0){
			endIx = cnty.length();
		}
		res = cnty.substring(0, endIx);		
		return res;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		//w.seek();
		System.out.println(w.getProv("203.88.203.2"));

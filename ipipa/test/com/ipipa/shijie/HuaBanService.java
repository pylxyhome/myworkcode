...
public class HuaBanService {
	...
	private static ThreadPoolExecutor writePool = new ThreadPoolExecutor(
			30, 60, 1000*2, TimeUnit.DAYS, writeQueue);
	/**
	 * 查询产品Map
	 */
	...
	public static void main(String[] args) throws IOException {
		...
		InputStream userStream = ClassLoader.getSystemResourceAsStream("user.properties");
		userProperties.load(userStream);
		userTotal = Integer.parseInt(userProperties.getProperty("Total"));
		/*
		InputStream catStream = ClassLoader.getSystemResourceAsStream("category.properties");
		Properties catProperties = new Properties();
		catProperties.load(catStream);
		*/
		
		InputStream keyStream = ClassLoader.getSystemResourceAsStream("keys_2.properties");
		Properties keyProperties = new Properties();
		keyProperties.load(keyStream);
		int keyTotal = Integer.parseInt(keyProperties.getProperty("Total"));
		
		HuaBanService huaban = new HuaBanService();
	...
	public  void seach(String word,Long gepageNoNo,int cid){
		System.out.println("Seach Word:"+word);
	...
	public static void check(String id){
        ...
        if(status != null && status.equals("1")){
        	...
        	save(json.getString("tags"), json.getString("tags"), json.getString("key"), json.getString("info"),0);
        }
	}
	
	
	public static void save(String content,String title,String imageUrl,String pageUrl,int cid){
		Map parameters = new HashMap();
		...
		doPost(ipipa+"/services/service.php?m=collectshare&a=save", parameters, "utf-8");
	}
...
//	            logger.error("网络故障", e);
	 ...
	 public class HuaBanThread implements Runnable{
		 ...
		 public HuaBanThread(Item itemObject,int cid){
			...
			public void run() {
				// TODO Auto-generated method stub
				String titile = itemObject.getTitle().substring(0, itemObject.getTitle().indexOf("<span"));
				save(itemObject.getDesc(), titile, itemObject.getPicUrl(), itemObject.getDetailUrl(),cid);
			}
